...
async function default_1(
	...
	log_1.default.info(`The Lounge ${chalk_1.default.green(
		helper_1.default.getVersion()
	)} \
(Node.js ${chalk_1.default.green(
		process.versions.node
	)} on ${chalk_1.default.green(process.platform)} ${process.arch})`);
	log_1.default.info(
		`Configuration file: ${chalk_1.default.green(
			config_1.default.getConfigPath()
		)}`
	);
	const staticOptions = {
		redirect: false,
	...
	if (options.dev) {
		(
			await Promise.resolve().then(() =>
				__importStar(require("./plugins/dev-server"))
			)
		).default(app);
	}
	app.set("env", "production")
		...
		.use(
			express_1.default.static(
				utils_1.default.getFileFromRelativeToRoot("public"),
				staticOptions
			)
		)
		.use(
			"/storage/",
			express_1.default.static(
				config_1.default.getStoragePath(),
				staticOptions
			)
		);
	...
	app.get("/packages/:package/:filename", (req, res) => {
		...
		if (
			!packageFile ||
			!index_1.default.getFiles().includes(`${packageName}/${fileName}`)
		) {
			return res.status(404).send("Not found");
		}
	...
	if (
		config_1.default.values.public &&
		(config_1.default.values.ldap || {}).enable
	) {
		log_1.default.warn(
	...
	if (!config_1.default.values.https.enable) {
		...
		const keyPath = helper_1.default.expandHome(
			config_1.default.values.https.key
		);
		const certPath = helper_1.default.expandHome(
			config_1.default.values.https.certificate
		);
		const caPath = helper_1.default.expandHome(
			config_1.default.values.https.ca
		);
		if (!keyPath.length || !fs_1.default.existsSync(keyPath)) {
			log_1.default.error(
				"Path to SSL key is invalid. Stopping server..."
			);
			process.exit(1);
		}
		if (!certPath.length || !fs_1.default.existsSync(certPath)) {
			log_1.default.error(
				"Path to SSL certificate is invalid. Stopping server..."
			);
			process.exit(1);
		}
		if (caPath.length && !fs_1.default.existsSync(caPath)) {
			log_1.default.error(
				"Path to SSL ca bundle is invalid. Stopping server..."
			);
			process.exit(1);
		}
		const createServer = (
	...
	let listenParams;
	if (
		typeof config_1.default.values.host === "string" &&
		config_1.default.values.host.startsWith("unix:")
	) {
	...
	server.listen(listenParams, () => {
		if (typeof listenParams === "string") {
			log_1.default.info(
				"Available on socket " + chalk_1.default.green(listenParams)
			);
		} else {
			const protocol = config_1.default.values.https.enable
				? "https"
				: "http";
		...
		sockets.on("connect", (socket) => {
			// eslint-disable-next-line @typescript-eslint/restrict-template-expressions
			socket.on("error", (err) =>
				log_1.default.error(`io socket error: ${err}`)
			);
		...
		const defaultTheme = themes_1.default.getByName(
			config_1.default.values.theme
		);
		if (defaultTheme === undefined) {
			log_1.default.warn(
				`The specified default theme "${chalk_1.default.red(
					config_1.default.values.theme
		...
		new identification_1.default((identHandler, err) => {
			if (err) {
				log_1.default.error(
					`Could not start identd server, ${err.message}`
				);
				process.exit(1);
			} else if (!manager) {
				log_1.default.error(
		...
		const exitGracefully = async function () {
			...
			if (config_1.default.values.prefetchStorage) {
				...
				(
					await Promise.resolve().then(() =>
						__importStar(require("./plugins/storage"))
					)
				).default.emptyDir();
			}
		...
		if (config_1.default.values.prefetchStorage) {
			Promise.resolve()
				.then(() => __importStar(require("./plugins/storage")))
				.then(({ default: storage }) => {
					storage.emptyDir();
				})
				.catch((err) => {
					log_1.default.error(
						`Could not clear storage folder, ${err.message}`
					);
				});
		}
...
function getClientSecure(socket) {
	...
	if (
		config_1.default.values.reverseProxy &&
...
function addSecurityHeaders(req, res, next) {
	...
	if (
		config_1.default.values.prefetchStorage ||
		!config_1.default.values.prefetch
	) {
		policies.push(
...
function indexRequest(req, res) {
	...
	return fs_1.default.readFile(
		utils_1.default.getFileFromRelativeToRoot("client/index.html.tpl"),
		"utf-8",
		(err, file) => {
			if (err) {
				throw err;
...
function initializeClient(socket, client, token, lastMessage, openChannel) {
	...
	socket.on("network:edit", (data) => {
		...
		const network = lodash_1.default.find(client.networks, {
			uuid: data.uuid,
		});
		if (!network) {
			return;
	...
	const sendSessionList = () => {
		...
		const sessions = lodash_1.default.map(
			client.config.sessions,
			(session, sessionToken) => {
				return {
					current: sessionToken === token,
					active: lodash_1.default.reduce(
...
function getClientConfiguration() {
	...
	if (!config.lockNetwork) {
		config.defaults = lodash_1.default.clone(
			config_1.default.values.defaults
		);
	} else {
		// Only send defaults that are visible on the client
		config.defaults = lodash_1.default.pick(
			config_1.default.values.defaults,
...
function getServerConfiguration() {
	return {
		...
		...{ stylesheets: index_1.default.getStylesheets() },
	};
}
function performAuthentication(data) {
	...
	const authCallback = (success) => {
		...
		if (!success) {
			if (!client) {
				log_1.default.warn(
					`Authentication for non existing user attempted from ${chalk_1.default.bold(
						getClientIp(socket)
					)}`
				);
			} else {
				log_1.default.warn(
					`Authentication failed for user ${chalk_1.default.bold(
						data.user
					)} from ${chalk_1.default.bold(getClientIp(socket))}`
				);
			}
...
function reverseDnsLookup(ip, callback) {
	...
	try {
		dns_1.default.reverse(ip, (reverseErr, hostnames) => {
			...
			dns_1.default.resolve(
				hostnames[0],
				net_1.default.isIP(ip) === 6 ? "AAAA" : "A",
